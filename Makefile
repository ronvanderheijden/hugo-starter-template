...
build: ## Build the static site
	docker compose run --rm hugo hugo --minify --destination public
...
new: ## Create new content (usage: make new path=posts/my-example-post.md)
	@test -n "$(path)" || { printf '%s\n' "path is required. Usage: make new path=posts/my-example-post.md"; exit 1; }
	docker compose run --rm hugo hugo new "$(path)"
